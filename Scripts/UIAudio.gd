...
const HOVER_DUR   := 0.055
const CLICK_DUR   := 0.030
...
func _ready():
	...
	# Pre-bake click so web has the buffer ready before first press
	_click_stream = _make_click()
...
func _make_sine(freq: float, duration: float, volume: float) -> AudioStreamWAV:
	...
	for i in range(n):
		...
		var env: float  = pow(1.0 - float(i) / float(n), 2.5)
		var s: int      = int(clamp(sin(TAU * freq * t) * env * volume, -1.0, 1.0) * 32767.0)
		data[i * 2]     = s & 0xFF
		data[i * 2 + 1] = (s >> 8) & 0xFF
	return _to_wav(data)
...
func _make_click() -> AudioStreamWAV:
	...
	for i in range(n):
		var t: float   = float(i) / SAMPLE_RATE
		var env: float = exp(-float(i) / float(n) * 12.0)
		var sig: float = sin(TAU * 520.0 * t) * 0.6 + sin(TAU * 1040.0 * t) * 0.4
		var s: int     = int(clamp(sig * env * 0.85, -1.0, 1.0) * 32767.0)
		data[i * 2]     = s & 0xFF
		data[i * 2 + 1] = (s >> 8) & 0xFF
	return _to_wav(data)
